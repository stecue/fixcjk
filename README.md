...
3. 由于脚本注入位置的差异，部分网页的部分内容（比如本项目在 GitHub 上的[中文 Readme 文件](https://github.com/stecue/fixcjk/blob/master/README.md)）在有的浏览器中可能需要手动刷新（`F5`）才能见效。
4. 类似问题3，对于某些动态生成的内容在网页载入完成时可能尚未被修正。此时可尝试单击页面内任意一点；脚本将重新检查所有元素。
